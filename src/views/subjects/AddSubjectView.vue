...
<template>
    <v-container>
        ...
        <v-row>
            ...
            <v-col md="8">
                <v-card class="elevation-5 ma-3" rounded="lg" style="height: auto; max-width: 1000px; min-width: 30vh">
                    <v-container>
                        ...
                        <v-form ref="form" class="ma-2">
                            ...
                            <v-text-field v-model="thaiName" :rules="nameRules" variant="outlined" rounded="lg"
                                class="small-input"></v-text-field>
                            <p style="font-size: 1.5vh">ชื่อหลักสูตร (อังกฤษ)</p>
                            <v-text-field v-model="engName" :rules="nameRules" variant="outlined"
                                rounded="lg"></v-text-field>
                            <p style="font-size: 1.5vh">รหัสหลักสูตร</p>
                            <v-text-field v-model="id" :rules="nameRules" variant="outlined"
                                rounded="lg"></v-text-field>
                            <p style="font-size: 1.5vh">ชื่อปริญญา</p>
                            <v-text-field v-model="select1" :rules="nameRules" variant="outlined"
                                rounded="lg"></v-text-field>
                            <p style="font-size: 1.5vh">ชื่อปริญญา ( อังกฤษ)</p>
                            <v-text-field v-model="engDegreeName" :rules="nameRules" variant="outlined"
                                rounded="lg"></v-text-field>
                            <p style="font-size: 1.5vh">สาขาวิชา</p>
                            <v-select v-model="select2" :items="branchOptions" variant="outlined"
                                rounded="lg"></v-select>
                    ...
                    <v-container>
                        ...
                        <v-form ref="form" class="ma-2">
                            ...
                            <v-combobox v-model="select3" :items="userOptions" variant="outlined"
                                rounded="lg"></v-combobox>
                            <!-- curriculumStore.currentCurriculum?.coordinators -->

                            <v-card style="border-color: #bdbdbd" variant="outlined" rounded="lg"
                                v-for="(curriculum, index) in coordinator" :key="curriculum.id || index"
                                ...
                                <v-row>
                                    ...
                                    <v-col class="d-flex justify-end" cols="auto">
                                        <v-btn color="red" variant="text" style="height: auto" class="circular-btn"
                                            icon="mdi-minus" @click="removeCoordinator(curriculum.id)">
                                        </v-btn>
                                    </v-col>
                                </v-row>
                            </v-card>
                            ...
                            <v-row class="justify-center">
                                <v-btn icon="mdi-plus" class="ma-8 rounded-circle" size="40px" variant="outlined"
                                    @click="addc"></v-btn>
                            </v-row>
                            <v-row class="justify-end mt-8">
                                <v-btn @click="reset" variant="plain" color="error">ล้าง</v-btn><v-btn @click="saveC"
                    ...
                    <v-container>
                        ...
                        <v-form ref="form" class="ma-2" v-for="(form, index) in forms" :key="index">
                            ...
                            <v-text-field v-model="form.description" :rules="nameRules" variant="outlined" rounded="lg"
                                class="small-input"></v-text-field>
                            <p style="font-size: 1.5vh">ผลลัพธ์การเรียนรู้ ตามมาตรฐาน คุณวุฒิฯ</p>
                            <v-select v-model="form.select5" :items="items4" variant="outlined" rounded="lg"></v-select>
                        </v-form>
                        <v-row class="justify-center">
                            <v-btn icon="mdi-plus" class="ma-4 rounded-circle" size="40px" variant="outlined"
                                @click="addForm"></v-btn>
                            <v-btn color="error" icon="mdi-minus" class="ma-4 rounded-circle" size="40px"
                                variant="outlined" @click="removeForm"></v-btn>
                        </v-row>
                        <v-row class="justify-end mt-8">
                            <v-btn @click="reset" variant="plain" color="error">ล้าง</v-btn><v-btn @click="saveC"
                                variant="plain">บันทึก</v-btn></v-row>
